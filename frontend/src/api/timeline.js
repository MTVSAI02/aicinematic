// 백엔드 Timeline API. (스토리보드 기반 — scene order/duration 조회·저장)
// 공통 fetch 래퍼(utils/request.js) 사용. 실패 시 error.detail 보존.
import { request, jsonBody } from '@/utils/request'

// GET /api/stories/{storyId}/timeline — 스토리 원본 순서, duration, totalDuration, readyStatus 등
export function getTimeline(storyId) {
  return request(`/api/stories/${storyId}/timeline`)
}

// PATCH /api/stories/{storyId}/timeline — 씬별 재생 길이(duration) 저장. 순서는 변경하지 않음.
