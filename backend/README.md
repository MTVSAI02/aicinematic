...
| POST | `/api/tts/scene` | 씬 TTS 생성 Job (**동기** Job — 즉시 completed, mock) |
...
- **실제 TTS 모델/AI 서버 호출 없음.** `audioUrl`은 항상 `null`(실제 음성 파일 없음). `tts_generate`는 **동기 Job**(`job_manager.run`)이라 즉시 `completed`. (씬 단위 짧은 합성이라 동기 — 길어지면 `run_async`로 전환)
