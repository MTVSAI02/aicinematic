...
class VoiceService:
    ...
    def __init__(self, voice_repo, character_repo):
    ...
    def delete_voice(self, voice_id: str) -> dict:
        if not self._voice_repo.delete(voice_id):
            raise VoiceNotFoundError()
        # 참조 해제: 이 voiceId를 쓰던 캐릭터의 voiceId를 null로 (배경 삭제와 동일 정책)
        self._character_repo.detach_voice(voice_id)
        return {"deleted": True, "voiceId": voice_id}


voice_service = VoiceService(voice_repository, character_repository)
